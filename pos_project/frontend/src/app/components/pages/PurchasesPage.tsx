...
export function PurchasesPage({ supplierInvoices }: PurchasesPageProps) {
  ...
  const [apiOrders, setApiOrders] = useState<Record<string, unknown>[]>([]);

  useEffect(() => {
    getPurchaseOrders().then((data: Record<string, unknown>[]) => {
      if (data.length > 0) setApiOrders(data);
    }).catch(() => {});
  }, []);
  ...
  const apiMapped = apiOrders.map(o => ({
    id: String(o.po_number || o.id || ''),
    supplier: String(o.supplier_name || o.supplier || ''),
    date: String(o.created_at || o.order_date || '').slice(0, 10),
    amount: Number(o.total_amount || o.grand_total || 0),
    status: String(o.status || 'pending'),
    items: Number(o.items_count || 0),
  }));
  const pagePurchases = apiMapped.length > 0 ? apiMapped : livePurchases;
