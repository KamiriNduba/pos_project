...
export function SuppliersPage({ products, supplierInvoices, onSupplierOrderCreated }: SuppliersPageProps) {
  ...
  const [apiSuppliers, setApiSuppliers] = useState<SupplierSummary[]>([]);

  useEffect(() => {
    getSuppliers().then((data: Record<string, unknown>[]) => {
      if (data.length > 0) {
        setApiSuppliers(data.map(s => ({
          id: Number(s.id),
          name: String(s.name || ''),
          contact: String(s.contact_person || 'Not captured'),
          email: String(s.email || ''),
          phone: String(s.phone || 'Not captured'),
          balance: 0,
          lastPurchase: '',
          totalPurchases: 0,
        })));
  ...
  const displaySuppliers = apiSuppliers.length > 0 ? apiSuppliers : suppliers;
